...
const TourCategories = () => {
  return (
    <section className="relative w-full overflow-hidden bg-white py-10 sm:py-12 md:py-14 lg:h-[500px] lg:py-0">
      {/* Background Vector */}
      <img
        src={Vector1052}
        alt=""
        className="pointer-events-none absolute inset-0 h-full w-full object-cover select-none opacity-70"
      />
      ...
      <img
        src={NotebookIcon}
        alt=""
        className="pointer-events-none absolute right-[14%] top-[82px] z-0 hidden w-[55px] select-none lg:block xl:right-[24%] xl:top-[76px] xl:w-[65px] 2xl:right-[27%] 2xl:top-[72px] 2xl:w-[72px]"
      />
      ...
      <img
        src={TravelIcon}
        alt=""
        className="pointer-events-none absolute bottom-[20px] left-[18px] z-0 hidden w-[50px] select-none md:block lg:bottom-[28px] lg:left-[18px] lg:w-[55px] xl:left-[25px] xl:w-[60px]"
      />

      {/* Heading */}
      <div className="relative z-10 px-4 text-center lg:pt-[48px]">
        <h4 className="font-serif text-[22px] font-normal italic leading-none text-[#173F52] sm:text-[24px] md:text-[26px] lg:text-[22px]">
          Wonderful Place For You
        </h4>

        <h2 className="mt-3 text-[34px] font-bold leading-none text-[#173F52] sm:text-[38px] md:text-[42px] lg:text-[40px]">
          Tour Categories
        </h2>
      </div>
          ...
          {categories.map((item, index) => {
            ...
            return (
              ...
              >
                ...
                <button className="mt-2 flex h-[34px] w-[112px] items-center justify-center gap-1.5 rounded-full border border-[#B8C8CC] bg-white text-[11px] font-normal text-[#143F4A] transition-all duration-300 hover:border-[#143F4A] hover:bg-[#143F4A] hover:text-white xl:h-[36px] xl:w-[118px] xl:text-[12px]">
                  Read More
                  <img
                    src={ArrowRight}
                    alt=""
        ...
        <div className="hidden snap-x snap-mandatory gap-4 overflow-x-auto px-4 scrollbar-hide md:flex lg:hidden">
          {categories.map((item, index) => (
            ...
            >
              ...
              <button className="mx-auto mt-3 flex h-[42px] w-[150px] items-center justify-center gap-2 rounded-full border border-gray-300 bg-white text-[14px] text-[#143F4A] transition-all duration-300 hover:bg-[#143F4A] hover:text-white sm:h-[44px] sm:w-[160px]">
                Read More
                <img
                  src={ArrowRight}
                  alt=""
          ...
          {categories.map((item, index) => (
            <div key={index} className="group w-full flex-none snap-center">
              <div className="mx-auto max-w-[320px]">
                ...
                <h3 className="mt-4 text-center text-xl font-bold text-[#143F4A]">
                  {item.title}
                </h3>

                <button className="mx-auto mt-3 flex h-[42px] w-[150px] items-center justify-center gap-2 rounded-full border border-gray-300 bg-white text-[14px] text-[#143F4A] transition-all duration-300 hover:bg-[#143F4A] hover:text-white">
                  Read More
                  <img
                    src={ArrowRight}
                    alt=""
